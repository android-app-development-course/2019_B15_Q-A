...
import zhj.viewpagerdemo.questionData;
...
public class TabFragment1 extends Fragment {
    ...
    public void initdata() {
        //初始化recycleview的数据
        //这里就是从数据库中获取数据
        BmobQuery<questionData> query = new BmobQuery<questionData>();
...
//请求图片数据(只有图片链接),返回一个CartoonsListEntity映射数组
        ...
        query.findObjects(new FindListener<questionData>() {

            @Override

            public void done(List<questionData> list, BmobException e) {
                ...
                if (e == null) {
                    ...
                    int n = list.size();
                    for (int i = 0; i < n; i++) {
                        mDatas.add("价值为"+list.get(i).getMark()+"金币") ;
                    }
                    for (int i = 0; i < n; i++) {
                        mnames.add(list.get(i).getQuestioner()) ;
                    }
                    for (int i = 0; i < n; i++) {
                        mtitles.add(list.get(i).getTitle()) ;
                    }
                    for (int i = 0; i < n; i++) {
                        mcontents.add(list.get(i).getDetail()) ;
                    ...
                    //失败直接打印即可
                    Toast.makeText(mRecyclerView.getContext(),"发生异常",Toast.LENGTH_LONG).show();
                }
